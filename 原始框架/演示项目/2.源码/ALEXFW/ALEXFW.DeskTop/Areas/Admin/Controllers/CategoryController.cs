...
namespace ALEXFW.DeskTop.Areas.Admin.Controllers
{
    public class CategoryController : EntityController<Category>
    {
        ...
        {
            var user = Session["AdminLogin"] as Entity.UserAndRole.Admin;

            var action = id == Guid.Empty ? "新增" : "修改";

            //添加一条日志 
            var logContext = EntityBuilder.GetContext<UserLog>();
            var log = logContext.Create();
            log.UserName = user.Username;
            log.RoleName = user.Group.ToString();
            log.Logs = action + "产品分类";
            log.CreateDate = DateTime.Now;
            await logContext.AddAsync(log);
